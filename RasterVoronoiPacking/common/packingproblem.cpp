...
bool PackingProblem::savePuzzle(QString fileName) {
	QFile data(fileName);
	if (data.open(QFile::WriteOnly)) {
		QTextStream out(&data);
		out << "// Automaticly generated by Puzzle Converter\n\n";

		out << "REC_CONTAINER\n";
		std::shared_ptr<Container> container = *ccbegin();
		QRectF containerBound = container->getPolygon()->boundingRect();
		out << qRound(containerBound.left()) << ", " << qRound(containerBound.top()) << "\n";
		out << qRound(containerBound.right()) << ", " << qRound(containerBound.bottom()) << "\n";

		int count = 1;
		for (auto it = this->cpbegin(); it != this->cpend(); it++, count++) {
			std::shared_ptr<Piece> curPiece = *it;
			out << "\n#PIECE " << count << "\n";
			out << "POLYGON " << curPiece->getMultiplicity();
			for (auto convexIt = curPiece->cConvexbegin(); convexIt != curPiece->cConvexend(); convexIt++) {
				out << "\n";
				std::shared_ptr<Polygon> curPartition = *convexIt;
				for (auto polIt = curPartition->begin(); polIt != curPartition->end(); polIt++) {
					QPointF curPt = *polIt;
					out << qRound(curPt.x()) << ", " << qRound(curPt.y()) << "\n";
				}
			}
		}
	}
