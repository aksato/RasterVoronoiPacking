...
void PackingClusterThread::run() {
	...
	if (parameters.getInitialSolMethod() == RASTERVORONOIPACKING::BOTTOMLEFT)  {
		...
		minSuccessfullSol = ExecutionSolutionInfo(curLength, (qreal)curLength * (qreal)containerHeight, 0.0, 1, seed, ExecutionSolutionInfo::ProblemType::StripPacking);
	...
	while (QDateTime::currentDateTime().msecsTo(finalTime) / 1000.0 > 0 && (parameters.getIterationsLimit() == 0 || totalItNum < parameters.getIterationsLimit()) && !m_abort) {
		...
		if (!parameters.isFixedLength()) {
			...
			if (success) {
				numLoops = 1;
				bestSolution = threadSolution; clusterSolver->declusterSolution(bestSolution); minSuccessfullSol = ExecutionSolutionInfo(curLength, (qreal)curLength * (qreal)containerHeight, getTimeStamp(parameters.getTimeLimit(), finalTime), totalItNum, seed, ExecutionSolutionInfo::ProblemType::StripPacking);
				curRealLength = (1.0 - rdec)*(qreal)solver->getCurrentWidth();
				curLength = qRound(curRealLength);
